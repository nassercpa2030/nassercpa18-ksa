...
class AircallIntegration(http.Controller):
    ...
    @http.route(
        '/aircall/webhook', auth='public', methods=['POST'], type='json', csrf=False)
    def webhook_listener(self, **kw):
        """Used to retrieve json payload directly instead of json-rpc formatted json."""
        json_payload = dict(http.request.dispatcher.jsonrequest)
        is_enabled_sms = request.env['ir.config_parameter'].sudo().get_param(
            'cit_aircall_api_integration.enable_sms')
        if is_enabled_sms:
            request.env["aircall.webhook"].message_events(json_payload)
        if 'token' not in json_payload:
            _logger.warning(
                'Received malformed json payload at webhook endpoint from {}'.format(
