...
class ProjectProject(models.Model):
    ...
    sale_order_id = fields.Many2one(comodel_name='sale.order', string="Sales Order", store=True, ondelete='set null')
    ...
    paid_percent = fields.Float(string="Paid Percent", compute="_compute_paid_percent", store=True)
    is_system_user = fields.Boolean(compute='_compute_is_system_user', store=False)
    stage_history_ids = fields.One2many(comodel_name='project.project.stage.history', inverse_name='project_id')
...
class SaleOrder(models.Model):
    ...
    first_line_name = fields.Char(string="First Line Description", compute="_compute_second_line_name", store=True)
    ...
    invoice_attachements_ids = fields.Many2many('ir.attachment', 'sale_order_invoice_attachment_rel', 'sale_order_id',
                                                'attachment_id', string='Invoice Attachments',
                                                compute='_compute_invoice_attachments', store=True)
    is_journal_state_not_posted = fields.Boolean(compute='_compute_is_journal_state_not_posted', string='Journal State',
                                                 default=True)
    ...
    user_id = fields.Many2one('res.users', string="Manager", compute='_compute_user_id', store=True, readonly=False,
                              precompute=True, index=True, tracking=2, domain="[('company_ids', '=', company_id)]")
