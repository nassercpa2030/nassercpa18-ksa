...
class ProjectProject(models.Model):
    ...
    sale_person= fields.Text(string="Salesperson", related='sale_order_id.broker_id.name',readonly=False)
   ...
   #file_state_history = fields.Char(compute='_compute_project_files_state', string='File_state History')
    ...
    def _compute_journal_entry_count(self):
        for order in self:
            order.journal_entry_count = self.env['account.move'].search_count(
                [('sale_order_id', '=', order.id), ('move_type', '=', 'entry'), ('journal_id', 'in', [17,37, 32, 33])])
    ...
    def action_view_journal_entries(self):
        ...
        return {
            ...
            'domain': [('sale_order_id', '=', self.id), ('move_type', '=', 'entry'), ('journal_id', 'in', [17,37, 32, 33])],
