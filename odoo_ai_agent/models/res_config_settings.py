...
class ResConfigSettings(models.TransientModel):
    ...
    reset_period_after_complete = fields.Integer(string="Reset Period After Completion (in Minutes)", config_parameter="copilot.reset_period_after_complete", default=10)
    reset_period_unit = fields.Selection(
        ...
        config_parameter="copilot.reset_period_unit",
        string="Reset Unit",
        default='minutes',
        required=True
    )
    ...
    def action_view_ai_modal(self):
        return {
            "type": "ir.actions.client",
            "tag": "odoo_ai_agent.open_ai_modal",
        }
    ...
    def get_ai_agent(self):
        try:
            ...
            for ai_agent in ai_agents:
                ...
                ai_agent_group_ids = list()
                ai_agent_groups = ai_agent.get('ai_agent_group_ids', [])
                if ai_agent_groups:
                    for group in ai_agent_groups:
                        ai_agent_group_id = self.env['ai.agent.group'].sudo().search([('code', '=', group.get('code'))], limit=1)
                        if not ai_agent_group_id:
                            ai_agent_group_id = self.env['ai.agent.group'].sudo().create(group)
                        ai_agent_group_ids.append(ai_agent_group_id.id)
                    val['ai_agent_group_ids'] = [(6, 0, ai_agent_group_ids)]
                ...
                important_info = ai_agent.get('agent_important_info')
                if important_info:
                    val['icon_text'] = important_info[0]  # First character
                    val['icon_link'] = important_info[1]  # URL
