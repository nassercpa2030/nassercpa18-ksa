...
from odoo.addons.odoo_ai_agent.models.call_back import search_internet_and_interpret_results_with_llm, DecisionMakingAgent
"""
ClassString = """
...
class AgentResponseHistory(models.Model):
    ...
    cost = fields.Integer(string="Cost", default=0)

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            agent_id = vals.get('copilot_agent_id')
            if agent_id:
                agent = self.env['copilot.agent.dashboard'].browse(agent_id)
                if agent.exists():
                    vals['cost'] = agent.cost
    ...
    def get_lat_long_from_ip(self, ip):
        try:
    ...
    def _get_server_answer(self, role, step, max_steps, current_step_output, functional_response):
        ...
        url = config_pram.get('redirect_url') + '/get/api-v2/get-agent-response'

        environ = request.httprequest.environ
        ip = environ.get('REMOTE_ADDR')
        geo = self.get_lat_long_from_ip(ip)
        ...
        response = requests.post(url, json=data, headers=header.get_headers())
    ...
    def create_response_line(self, answer):
        ...
        show_query = answer.get('show_query')
        execution_control_msg = answer.get('execution_control_msg')
        ...
        if execution_control_msg:
            full_response = execution_control_msg
            ...
            try:
                ...
                full_response = ""
                if final_step and not agent_code in ["004","008","002","006","007","011","012"]:
        ...
        val = {
            ...
            "final_step": answer.get("final_step"),
            "python_functions": answer.get('python_functions') if show_query else False,
            "show_query": show_query if show_query else False,
        ...
        if final_step and agent_code in ["004","008","002","006","007","011","012"]:
    ...
    def store_formated_data(self, output_dictionary, agent_code):
        ...
        if agent_code == "010":
            matching_info = output_dictionary.get('matching_info')
            if matching_info:
                return [matching_info, output_dictionary]
            else:
    ...
    def code_preprocessing(self, code_to_execute):
        code_to_execute = code_to_execute.replace("self.env", "request.env")
        executable_code = self.import_handler()
        executable_code += "\n\n"
        executable_code += ClassString
    ...
    def execute_python_function_multi(self, code_to_execute, prev_step_executed_data=None):
        ...
        executable_code = self.code_preprocessing(code_to_execute)
    ...
    def fetch_last_agent_response(self, agent_id, fields=None):
        data = []
        xml = self.sudo().env.ref('odoo_ai_agent.agent_response_history_form_view_custom_design').arch_db
        ...
        for id in agent_id:
            ...
            if response:
                steps_data = response.agent_response_history_step_ids.read(['id','has_user_confirmation', 'step', 'text_response', 'output_response', 'text', 'title', 'current_role', 'final_step', 'template_id','show_query'])
